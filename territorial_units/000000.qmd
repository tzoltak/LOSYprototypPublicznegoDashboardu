...
```{ojs}
//| echo: false
status = await FileAttachment("../data/000000-status.json").json()
```
...
statusD = [].concat(
  ...
  .map( (g) => g.wartosc.w.map(
    (x) => Object.assign({}, x, { TERYT: "000000",
                                  JST: "Polska",
                                  typ_szk: g.typ_szk,
...
```{ojs}
//| echo: false
bezrobocie = await FileAttachment("../data/000000-bezrobocie.json").json()
```
...
```{ojs}
//| echo: false
sr_wynagr = await FileAttachment("../data/000000-sr_wynagr.json").json()
```
...
```{ojs}
//| echo: false
matura_zdana = await FileAttachment("../data/000000-matura_zdana.json").json()
```
...
```{ojs}
//| echo: false
dyplom_zaw = await FileAttachment("../data/000000-dyplom_zaw.json").json()
```
...
```{ojs}
//| echo: false
typ_szk_kont = await FileAttachment("../data/000000-typ_szk_kont.json").json()
```
...
```{ojs}
//| echo: false
dyscyplina_kont = await FileAttachment("../data/000000-dyscyplina_kont.json").json()
```
